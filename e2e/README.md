...
| `preflight/client-shape`       | the client the stage serves was built from the sources this harness reads, comparing the build stamp's client and shared trees against the checkout. One HTTP read, spends nothing      |
...
| `scenarios/whole-stack-restart-reconnect` (M) | host reboot **and the broadcaster comes back** inside the recovery window → the restarted engine counter is taken rather than swallowed as duplicates, and the same broadcast stays live. The live proof of finding 1 of the 2026-09-05 cross-provider review, which I cannot see because its publisher never reconnects |

Both restart scenarios run in every full sitting and neither needs an arming. `pnpm e2e:reconnect-after-reboot`
runs the gates and M alone, which is the cheapest way to read it after a change to the recovery path.
