...
import type { ContinuationTopology, ReadinessProbeId, TopologyServiceRole } from './topology.js';
...
interface ReadinessContainerBinding {
  id: string;
  name: string;
}
...
interface ReadinessControlClock {
  now(): number;
...
export class FixtureReadinessControlExecutor implements ReadinessControlExecutor {
  ...
  private async storage(maxResponseBytes: number): Promise<ControlResult> {
    ...
    const expectedBatchHash = `sha256:${createHash('sha256')
      .update(normalizedBatchId(this.input.postageBatchId))
      .digest('hex')}`;
  ...
  private async falseCodec(maxResponseBytes: number): Promise<ControlResult> {
    const raw = await this.nodeControl('browser', BROWSER_CONTROL_SCRIPT, { mode: 'false-codec' }, maxResponseBytes);
    const result = {
  ...
  private async nodeControl(
    ...
    const result = await this.command.run('docker', ['exec', this.binding(role).id, 'node', '-e', script, payload]);
