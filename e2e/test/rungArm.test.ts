...
import { armState, INSTRUMENT_UNPROVEN, MOVED_OFF_A_DEAD_RUNG, rungArmState } from './helpers/browserArmFixtures.js';
...
describe('whether a run is a viewer whose rung went quiet', () => {
  ...
  it('refuses a run whose instrument was never shown able to report a failure', () => {
    const unproven = parseBrowserArmState(rungArmState({ instrumentProofs: INSTRUMENT_UNPROVEN }));

    assert.match(String(rungArmRefusal(unproven, CLEAN_ARM)), /restatement of the launch flags rather than evidence/);
  });
