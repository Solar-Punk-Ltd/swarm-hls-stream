 ...
 * its stream has not started, so they share one SWR key and one poll rather than running two against
 * the gateway. The gateway is part of the key so that a switch starts a fresh fetch rather than
 * inheriting the previous node's answer: `isLoading` is then true again while the new node is being
 * asked, and an `error` belongs to the node now selected instead of the one the viewer has left.
 *
