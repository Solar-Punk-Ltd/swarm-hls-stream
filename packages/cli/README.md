...
| `pnpm stamp:buy`     | Buy a stamp for one rung: `<rung> [amount] [depth]`   |
...
The rung is required and validated against `BEE_PUBLISHERS`. An unknown rung, a missing rung, or an unsplit config all fail before any network call, listing the rungs that *are* configured:
