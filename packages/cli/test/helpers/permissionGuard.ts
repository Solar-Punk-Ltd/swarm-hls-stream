 ...
 *
 * `false` runs the case, a string skips it and carries the reason into the report. Skipped rather
 * than silently passed on purpose: a case whose precondition is missing must never be counted as
 ...
 */
export const SKIP_WITHOUT_PERMISSION_ENFORCEMENT: string | false = unwritableIsEnforceable(process.geteuid?.())
  ? false
