...
| `SRS_LIFECYCLE_VERSION`  | _(empty)_            | `1` turns on the managed SRS lifecycle, where this service drives the engine's ingest and rungs instead of following them. Needs `ADMIN_API_URL` and `ENGINE=srs`. The SRS container reads the same variable to decide whether its ingest source writes HLS, so both services have to be given it |
| `SRS_UPLOADER_ID`        | _(empty)_            | This uploader's own name in the managed lifecycle, 1 to 200 letters, digits, dots, underscores, colons or hyphens. Required when `SRS_LIFECYCLE_VERSION` is set                                                                                                                                   |
