...
Test baseline is **145 uploader, 5 client**, with lint, typecheck and whole-tree prettier all clean.
...
**Next task: the client half of S2.2.** The uploader half landed in PR #34: all three puller calls route
through one `fetchWithTimeout` with a fresh `AbortSignal.timeout`, settable with `OME_FETCH_TIMEOUT_MS`
and defaulting to 10s, and an abort logs at error level. The acceptance criterion is met and proven with
the S0.6 fetcher, no network.
