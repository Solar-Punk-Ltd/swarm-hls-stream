...
**Where the code is.** `feat/ai-hardening` at `8cfb7e8`. Fifteen pull requests merged, each through the
...
2. **Test debt, now including TEST-23.** Nothing proves `retireSession` is needed in the re-announce
