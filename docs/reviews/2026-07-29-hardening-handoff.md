...
S2.1, SEC-11 and TEST-9. Three earlier PRs are merged, each through the review gate: S1.4 as #27,
...
Test baseline is **114 uploader, 5 client**, with lint, typecheck and whole-tree prettier all clean.
...
**Next task: S2.2**, timeouts on all four `fetch` call sites. It is next in the P0 order now that
