...
**PR #34 is open and unblocked, pending its second gate.** Branch `feat/fetch-timeouts`, S2.2's uploader
half. Its first gate ran six lenses and found a HIGH regression the change itself introduced, recorded as
**OBS-11**, and everything that gate raised is now fixed on the branch. Do not merge it before its second
gate runs, because the fixes below are a bigger change than the one the first gate reviewed.
...
Test baseline is **180 uploader, 5 client**, with lint, typecheck and whole-tree prettier all clean.
