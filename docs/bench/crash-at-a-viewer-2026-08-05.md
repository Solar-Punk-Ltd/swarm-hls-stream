# Two crashes, watched from a viewer's seat

**2026-08-05.** The first time anything in this project has broken under a browser that was watching.
Both runs instrument-sound. `browser:crash`, which injects the fault itself over the docker socket so
...
uploader's log. They answer whether the **publisher** did the right thing. Neither of the two things
below is visible from there.

## What the viewer got

| | gateway stopped 20.5s | uploader killed 15.3s |
| --- | ---: | ---: |
| picture kept moving for | 6.1s | 7.1s |
| then froze for | **30.6s** | **54.9s** |
| moved again, after the service returned | **16.2s** | **46.7s** |
| recovered without a reload | ✅ | ✅ |
| the client said | "Reconnecting to the stream" | "Waiting for the broadcast to continue" |
| behind live, before → after | 5.77s → 6.05s | 5.90s → 7.01s |
...
Both runs froze for much longer than the fault lasted, and in both the extra time is on this side.
...
**One run each.** Both agree with a mechanism read off their own request logs rather than assumed,
which is stronger than two bare numbers, and neither has been repeated.
...
**The third scenario in the catalog, an engine restart under a live publisher, has not been run.**

**Segment length was 0.25s throughout, and the client was the fixed one** from
[the loop fix](./the-loop-fixed-2026-08-05.md). Both runs sat at 5.8 to 5.9s behind live before their
