...
| 2026-09-03 | [✅✅✅ THE DATES RE-ANCHOR AT AN ENGINE RESTART, AND THE SUITES NOW ASSERT THE PLAYLIST CONTRACT THEMSELVES. Full gateway sitting on the uploader redeployed with both: 9 of 9 gates, 36 of 36 suites. The 1080p rung minted the new dating line at the restart, removing 17 s of lag, and 23 later announces across four rungs joined that one line. Seven suites fetched the playlists their broadcasts published and held them to sequence 0, a date on every segment and one-fragment steps. Found on the way: the engine's counter reset never armed a discontinuity on the SRS path, now it does, and a fifth log family says so. Still unobserved live: the gap left by segments SRS posts while the uploader is dead, which F reads after it has slid out of the window. 2.494 BZZ](sequence-zero-and-timestamps-2026-09-03.md) |  |
| 2026-09-03 | [✅✅✅ TWO FULL SITTINGS ON THE STAMPED, RENUMBERED UPLOADER. In-tab node: 9 of 9 gates and 33 of 33 suites, the first fully green sitting this project has had. Through the gateway the same afternoon: 9 of 9 and 32 of 33, and the one red is the harness, whose warmup summed segments across rungs and killed the uploader before the 1080p rung had uploaded anything, the ninth merged numerator to read as a fault. Fixed to wait per rung and proven live alone the same afternoon, H green with all four renditions kept, after the spend gate had correctly refused the first attempt because a 3 BZZ deposit landed mid-sitting. Every suite has now passed live on this uploader in both byte sources. About 1.8 BZZ in-tab, 2.486 through the gateway, of which the gateway node's own retrievals were 0.619](sequence-zero-and-timestamps-2026-09-03.md) | [V2 gateway](browser-quality-2026-09-03T06-32-01-420Z.md) [V4 gateway](browser-vod-2026-09-03T06-44-21-952Z.md) |
| 2026-09-03 | [✅✅✅ THE SEGMENT-LOSS GAP IS CLOSED AND PROVEN LIVE, TWICE. SRS posts each closed segment once and never retries, so the segments it closed while the uploader was dead were a hole the playlist promised as seamless, and F could not see it because it read after the join had slid out of the window. The uploader now infers the gap from the engine's own numbering, arms the break, counts the loss and writes a sixth arming line the harness counts on its own. F reads the join while it is in the window: two runs, four rungs each, one break per rung at MEDIA-SEQUENCE:0 across a two to three segment gap, contract held, then the same window a minute later with the join gone. Sequence zero held with SRS's counter at 0 and at 69. 0.100 and 0.109 BZZ](sequence-zero-and-timestamps-2026-09-03.md#the-segment-loss-gap-closed-and-proven-live-the-same-night-twice) |  |
| 2026-09-02 | [✅ ABEL'S OWN PAGE PLAYS OUR BROADCAST WITH NO GATEWAY, realtime 1.000 for four minutes, one stall, in-tab node only, once the recording it opened started at media sequence 0 (a fresh recording after an SRS restart, which is what every broadcast now guarantees). Two caveats travel with it: the page had prefetched most of the 505 s recording, so the capped minute most likely played from buffer, and the tab's WebSocket byte count reads 0 because his node runs in a SharedWorker that Playwright cannot see. An earlier attempt the same evening was inconclusive, its settle window opened before his player had started, and its files were dropped](weeb3-native-2026-09-02T10-26-15-831Z.md) |  |
| 2026-09-02 | [✅ V2 GREEN, FIRST TIME. The in-tab viewer (weeb-3 0.0.341001 in its SharedWorker) rode 1080p, was capped at 2800 kbps for 61 s through a cap that now reaches the worker, asked for 360p 3.0 s in, decoded it at 14.1 s, kept the picture moving at 0.928x, and climbed to 480p 29.4 s after the lift. 15 of 22 level-0 loads completed under the cap at a 2.5 s median, against 0 of 5 in the three red sittings. Observations: the bottom rung was choppy under Chrome's emulation (13 rebuffers, buffer 0.05 to 1.9 s) and the 7 abandoned attempts were all answered later, 2.5 MB of a capped link. Preflight 9/9, 0.183 BZZ](browser-quality-2026-09-02T12-52-16-340Z.md) |  |
| 2026-09-02 | [⛔ THE MORNING'S RED WAS THE INSTRUMENT. Under Chrome's emulated 2800 kbps cap our client on weeb-3 0.0.329001 read 3.4 to 4.2 s per 360p segment at 2x the bytes on a half-idle link; the same release, client, recording and driver under a REAL `tc` link proved at 330 KB/s reads **1.0 s at 1.1x the bytes with the link 70% full**. Chrome's emulation also never reaches a SharedWorker, so on 0.0.341001 three "held under the cap" readings were void, and the recorder passed its own blindness as "H0 holds". On the real link 0.0.341001 fetches 360p in 0.8 to 1.0 s and 1080p in 6.7 to 6.9 s, joins in 30 s, and one capped boot in two timed out. Three contracts of ours surfaced on the way: sequence zero, the hosted runtime, a secure context. 0 BZZ throughout](in-tab-throttle-probe-result-2026-09-02.md) |  |
| 2026-09-02 | [Registers three mechanisms for a capped in-tab node delivering nothing in 20s on a link that carries a segment in 0.63s: H1 hedge amplification at 3.0x or more, H2 idle load at 30% of the cap, H3 accounting exhaustion with the link left idle while the node waits on per-peer allowance, plus an instrument check that voids every capped figure if a 700 kbps cap does not hold idle inbound under 87.5 KB/s](in-tab-throttle-probe-prediction-2026-09-02.md) | 📋 plan |
...
| 2026-08-16 | [Our own broadcast **plays** in weeb-3's own page with zero off-shell bytes: realtimeRatio **1.0000** once moving over 214s, 24/24 segments at 1080p, 2 stalls and a 26.1s startup. This is the measurement the owner asked for on 2026-08-11, and it answers the "did not reach playback" line above](weeb3-native-arm-2026-08-16.md) |  |
