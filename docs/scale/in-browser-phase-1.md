...
deployment, on a node with no chequebook, which is why it could not spend. ⚠️ That is the **default**
state of a browser node, not a limit on one: see the correction at the end of section 6.
...
✅ **CONFIRMED 2026-08-10, and the cause has a name.** A second attempt reproduced it exactly and then
read the document state: `visibilityState: "hidden"`, `hasFocus: false`,
`navigator.userActivation.hasBeenActive: false`, and **`video.error: null`**. The automated pane runs
as a **permanently hidden document that has never received user activation**, so autoplay is forbidden
and media is suspended. **[OBSERVED]**

⛔⛔ **The failure mode is worth knowing because it impersonates the thing being measured.** With the
node fully connected at 200 peers, the playlist fetched **200**, the first segment fetched **200**, the
**second segment was `net::ERR_ABORTED`**, the buffer froze at **0.17 s**, `readyState` stuck at **1**,
and nothing reported an error. That is indistinguishable from "the browser node cannot keep up" unless
you check visibility. **Any sustained-playback number from an agent-driven pane is a fact about the
...
being slow. Attributing the gap to funding needs a funded **weeb-3** on this same laptop, which is
...
⛔⛔⛔ **CORRECTED 2026-08-15.** This paragraph read: _"it cannot settle, so bee skips peers ~37x per
chunk and it asks 38 peers where a funded one asks 1"_. **The 38 are loop iterations counted inside
...
treat it as unmeasured, not as 38x and not as 1x.** The honest bound available today is the one the
source gives for free: `RETRIEVE_CHECK_CONFIRMATION_PEERS = 6` peers per chunk **by design**, which is
a floor rather than an estimate, and the overdraft ladder sits on top of it by an unknown amount.

### ⭐⭐⭐ CORRECTION, from source, 2026-08-10: unfunded is the DEFAULT, not a limitation

**weeb-3 can issue real cheques.** It carries a complete wallet-connected economic path, and the
earlier framing here, which treated a browser node as inherently unable to spend, was wrong about the
capability while being right about what we measured. **[SOURCE]**

The gate is one function, `cheques_active()` in `lib.rs:412`, and it requires **four** things:

| condition                            | what supplies it                              |
| ------------------------------------ | --------------------------------------------- |
| a chequebook signer key              | generated in-browser, stored locally          |
| a 20-byte chequebook address         | `deploy_chequebook_with_payer(issuer, payer)` |
| a working web3 handle                | the injected browser wallet                   |
| ⛔ **a NON-ZERO chequebook balance** | `deposit_to_chequebook(...)`                  |

The `payer` comes from `connect_wallet_address()`, so **the connected browser wallet pays the gas**
and the node deploys its own chequebook. The same path also exposes `buy_postage_batch_with_payer`.

The gate is genuinely consulted rather than vestigial. In the refreshment handler at `lib.rs:3500`:

```rust
if *cheques_active_cache.lock().await == Some(true) && balance > REFRESH_RATE {
    let cheque_amt = balance - REFRESH_RATE;
    ...  // -> issue_handler -> SWAP_PROTOCOL -> a signed cheque
}
```

Three consequences worth carrying:

1. ⭐ **Funding does not replace pseudosettle, it caps the debt.** The cheque path is reached _through_
   the refreshment handler, and it only pays the excess **above** `REFRESH_RATE`. A funded node still
   free-rides the first 450000 with every peer.
2. ⛔⛔ **`cheques_active` is evaluated ONCE at startup** and cached (`lib.rs:3333-3340`). A viewer who
   funds a chequebook mid-session keeps pseudosettling until the page is reloaded. Any funded arm must
   fund **first** and load **second**, or it measures the unfunded path and looks like a null result.
3. ⭐ **This makes question 4 a clean experiment.** The comparison no longer has to be weeb-3 against a
   bee light node, which would have confounded implementation with funding. It is now **the same
   binary, the same laptop, the same peer table, one condition changed**.

⚠️ **What does NOT change: the supply side.** Funding buys a browser node better _retrieval_. It still
never calls `.accept()` on the retrieval protocol, so it still serves nothing and caches nothing for
anyone else. **All demand, no supply survives this correction intact.**

⛔ **The funded arm costs money** and is therefore not mine to start: it needs a chequebook contract
deployed (gas) and BZZ deposited into it. See question 4 in section 8 for the exact ask.
...
was _arithmetic_ and that the _measurement_ at concurrency 3 was **212 KB/s**. This section called
345 "measured, n=500".
...
⚠️ What is still true is the _shape_ of the caveat: these were **delivery** measurements taken with
...
| **4** | ⏸ **DEFERRED BY THE OWNER, 2026-08-10. No chequebook is being deployed and this is not to be re-proposed until they reopen it.** The question stands: does the 38x unfunded amplification carry to weeb-3, on top of the `RETRIEVE_CHECK_CONFIRMATION_PEERS = 6` floor the source already sets                                                                                                                                                                                                                                      | Preserved for whenever it reopens: both arms in **one sitting**, unfunded first with the wallet disconnected, then fund, **reload**, funded arm. They cannot be alternated, because once funded you cannot cheaply return to unfunded. ⛔ The reload is not optional, `cheques_active` is decided once at startup | ⛔⛔ **What deferring costs, stated plainly: "funding removes most of the tail" stays a PREDICTION.** The floor is stable at ~785 ms and the tail tracks the 194 ms overdraft ladder, which is why funding is the suspected lever, and nothing has tested it. Do not let the rest of this document read as though the picture is complete                                                                                 | ⏸ **deferred, no spend**                                                     |
